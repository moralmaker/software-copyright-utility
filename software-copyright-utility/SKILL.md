---
name: software-copyright-utility
description: >
...
metadata:
  ...
  author: moralmaker
  version: "1.7"
  repository: https://github.com/moralmaker/software-copyright-utility
---
...
- 操作手册草稿必须与官方《软件操作说明书样本》结构一致：封面（软件名称、版本号、“用户手册”）→ 目录 → `1 引言`（`1.1 编写目的`、`1.2 参考资料`、`1.3 术语和缩略词`）→ `2 软件概述`（`2.1 软件功能`、`2.2 软件运行`、`2.3 系统要求`）→ `3 系统使用`（每个真实页面/流程独立成节 `3.1`、`3.2`…）。顶级章节用阿拉伯数字编号（`1`、`2`、`3`），二级 `N.M`、三级 `N.M.K`，禁止使用中文大写序号（`一、二、三`）。`2.1 软件功能` 使用 `（1）（2）（3）` 编号列表概括核心功能；参考资料用《文档名》逐行列出；术语用“术语  解释”逐行列出；系统要求用简洁文字行说明最低/推荐配置。`3 系统使用` 每个模块以连续段落写清用途、入口、用户可见控件、操作动作、输入限制/异常提示与结果反馈，并在末尾保留可见截图预留文字与 `图3-X` 图注（X 为系统使用章节内连续编号）。
...
- 截图方式必须先让用户选择：Chrome DevTools MCP（Web）、微信小程序 wechat-devtools MCP（uni-app/Taro 编译到微信小程序）、Codex Computer Use、用户自行截图。项目是小程序应用时优先用 wechat-devtools MCP 在模拟器中运行并逐页截图。用户选完后，再检查当前 MCP / Computer Use 能力是否可用；如果用户说现在不截图、先跳过截图或截图失败，操作手册仍必须保留清晰可见的截图预留位置，正式 Word 中也要能看到。
...
- `screenshot-method`：操作手册截图前，用户必须在 Chrome DevTools MCP（Web）、微信小程序 wechat-devtools MCP（小程序应用）、Codex Computer Use、用户自行截图方式中选择一种；如果用户明确说“现在不截图/先跳过截图”，记录为 `skip`。
...
- 软件分类：应用软件/嵌入式软件/中间件/操作系统，默认为应用软件。
- 开发完成日期、首次发表日期：必须使用 YYYY-MM-DD 格式。
- 开发方式：独立开发/合作开发/委托开发/下达任务开发，默认独立开发（官方无“单独开发”）。
...
- 代码字体必须使用宋体（SimSun）、五号（10.5pt），页面为 A4，左右页边距约 3.17cm、上下约 2.2cm，与《源代码-模板》一致。
- 代码 Word 页眉为左对齐的单行“软件全称 版本号”，不使用表格、不添加“第 X 页”页码，页脚留空。
...
操作手册草稿必须与官方《软件操作说明书样本》结构一致：封面（软件名称、版本号、用户手册）+ 目录 + `1 引言`（编写目的/参考资料/术语和缩略词）+ `2 软件概述`（软件功能/软件运行/系统要求）+ `3 系统使用`（各真实页面/流程独立成节），可再补充 `4 常见问题解答`。章节用阿拉伯数字编号（`1`、`2`、`3` 与 `N.M`、`N.M.K`），禁止使用中文大写序号。`2.1 软件功能` 用 `（1）（2）` 编号列表概括核心功能；`1.2 参考资料` 用《文档名》逐行列出；`1.3 术语和缩略词` 用“术语  解释”逐行列出；`2.3 系统要求` 用简洁文字行说明最低/推荐配置。`3 系统使用` 每节以连续段落展开用途、入口、控件、操作、规则与反馈，末尾保留可见截图预留文字与 `图3-X` 图注；不得写成“进入方式：/页面内容：/操作步骤：”字段模板，也不使用 `1. 2. 3.` 项目符号列表堆信息。必须基于模型写入 `草稿/业务理解.json` 的 `manual_modules` 组织章节；`manual_sections` 只用于补充说明性段落，不应用来反复插入同一批功能模块。各功能章节必须写清页面用途、进入位置、用户看到的控件和数据、实际操作、输入限制或异常提示、操作结果和截图预留。语言要面向普通用户，说明“这个页面是干嘛的、用户怎么进入、用户点什么/填什么、操作后看到什么”，不要写代码实现、框架名称、接口封装、状态管理、异步队列等技术细节。撰写时由 agent 自行检查章节是否完整、内容是否过薄、语言是否过于技术化，并在草稿内部完成必要补写；完整草稿完成后只让用户做一次整体确认，确认前不得进入正式 Word/TXT 生成。
...
操作手册草稿完成后，先停止并让用户选择截图方式，按项目类型给出对应选项（小程序应用优先第 2 项）：

1. Chrome DevTools MCP：适合 Web / 浏览器项目，优先用于网页全页截图（工具前缀 `mcp__chrome_devtools__`）。
2. 微信小程序 wechat-devtools MCP：适合 uni-app / Taro 等编译到微信小程序的项目，在微信开发者工具模拟器中运行并逐页截图。**项目是小程序应用时必须优先选此项。**
3. Codex Computer Use：适合需要通过桌面应用或浏览器界面点击、切换、查看状态后截图的场景。
4. 用户自行截图：用户自己把 PNG/JPG/JPEG/WebP 图片放入 `软件著作权申请资料/用户截图/`，agent 只负责整理和引用。
...
python3 ${CLAUDE_SKILL_DIR}/scripts/confirm_stage.py \
  ...
  --method <chrome-devtools|wechat-devtools|computer-use|user-supplied|skip> \
  --note "<用户选择>"
```

然后按用户选择检查当前能力并执行：

- 选择 Chrome DevTools MCP：先用工具发现能力检查当前环境是否有 `mcp__chrome_devtools__` 的 `list_pages`、`take_snapshot`、`take_screenshot`。可用时，先 `list_pages` 确认当前浏览器页面，再按页面/路由截图保存到 `软件著作权申请资料/截图/`；不可用时停止，告知用户需要重新选择截图方式或手动提供截图。**若页面需要登录，先向用户索要账号密码（用 `fill` 自动登录）或请用户先在浏览器完成登录、确认后再截，不得用登录页顶替功能页。**
- 选择微信小程序 wechat-devtools MCP（小程序应用，如 uni-app / Taro）：标准流程如下（工具前缀 `mcp__wechat_devtools__`，server 名 `wechat-devtools`）：
  1. **编译小程序产物**：按项目脚本执行 `pnpm build:weapp`（或 `pnpm dev:weapp:<env>`），生成微信开发者工具可识别的 `dist/**/mp-weixin` 目录（uni-app 通常为 `dist/dev/mp-weixin`）。确认该 dist 目录已在微信开发者工具中导入（`project_list` 能查到对应 `projectPath`；未导入则让用户在微信开发者工具“导入项目”指向该 dist 目录，或用 `project_import`）。
  2. **确认环境**：`check_wechatide_status` 确认微信开发者工具已启动且已登录；`project_list` 拿到本项目的 `dist` 产物绝对路径（下称 `<dist>`）。
  3. **打开窗口启动模拟器**：`open_project_window`（`project=<dist>`，`windowMode="liteMode"`）。
  4. **逐页截图**：对操作手册的每个核心页面，先 `simulator_open_page`（`project=<dist>`、`page`=app.json 中的页面路径，如 `pages/orderList/index`，需要参数时带 `query`）导航到目标页面，再 `simulator_screenshot`（`project=<dist>`、`path`=`软件著作权申请资料/截图/<模块名>-NN.jpg`，可用 `wait` 等待渲染稳定）保存截图。首页通常直接 `simulator_screenshot` 即可。
  5. **登录态页面（必须停住等用户）**：截图前若页面需要登录（截图落在登录/注册/未授权页），必须先取得登录态，且**不得用登录页顶替功能页截图**。二选一并向用户说明：
     - **索取账号密码**：主动向用户索要该小程序的登录账号与密码，用 `automation_*`（`automation_element_action` 定位账号/密码输入框、`automation_fill` 填写、点击登录/完成验证）完成登录后再逐页截图。账号密码仅用于本次模拟器登录，**严禁写入**草稿、截图清单、操作手册或任何正式资料。
     - **请用户手动登录**：请用户直接在微信开发者工具模拟器里完成登录，等用户回复“已登录”后再继续截图。
     - 两种方式都无法获得登录态时，该模块保留可见截图预留文字，并在报告中说明“该模块需登录，暂未截图”。
  6. 截图文件名与操作手册 `图3-X` 对应；完成后运行 `capture_screenshots.py` 或直接生成 `截图/截图清单.json`，并把截图引用补入 `草稿/操作手册.md`。
...
- 软件名称和版本号是否一致
- 代码材料前30页、后30页页眉是否为左对齐“软件全称 版本号”（宋体、A4、无页码、无表格），且与《源代码-模板》格式一致
- 操作手册是否为封面 + 目录 + 阿拉伯数字章节（`1 引言` / `2 软件概述` / `3 系统使用`），页脚是否为“软件全称 版本号 + 第 X 页”，各 `3 系统使用` 模块是否带 `图3-X` 图注与可见截图预留
...
正式生成脚本必须重新读取 `草稿/申请表信息.md` 中已确认的“软件全称”和“版本号”，并用它们生成正式资料文件名、代码 Word 页眉和操作手册封面/页脚。代码材料页眉为左对齐“软件全称 版本号”（宋体、A4、无页码）。操作手册按《软件操作说明书样本》生成：封面（软件名称、版本号、用户手册）+ 目录 + 阿拉伯数字章节，正文页脚左侧为“软件全称 版本号”、右侧为“第 <页码> 页”，封面页不显示页码。若命令参数 `--software-name` / `--version` 与申请表字段不同，以申请表字段为准，并在 `正式资料/生成报告.md` 中记录提示。

输出：

- `正式资料/申请表信息.txt`
- `正式资料/计算机软件著作权登记申请表.docx`（按官方模板结构填写的登记申请表：软件基本信息、著作权人、权利说明、软件鉴别材料、软件功能和技术特点、申请人信息、代理人信息、声明；选项类字段用 ● 标注已选）
- 代码达到或超过 60 页：
...
- 操作手册截图前，需要用户在 Chrome DevTools MCP（Web）、微信小程序 wechat-devtools MCP（小程序应用）、Codex Computer Use、用户自行截图方式中选择一种；选择后再检查对应工具是否可用。

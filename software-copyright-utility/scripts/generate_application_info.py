...
# 官方「计算机软件著作权登记申请表」开发方式选项（不含"单独开发"）
DEVELOPMENT_SITUATION_OPTIONS = ("独立开发", "合作开发", "委托开发", "下达任务开发")


def normalize_development_situation(value: Any) -> str:
    text = str(value or "").strip()
    if not text or "待用户确认" in text:
        return "独立开发"
    # 兼容旧口径"单独开发"，统一映射为官方选项"独立开发"
    if "单独" in text:
        return "独立开发"
    for option in DEVELOPMENT_SITUATION_OPTIONS:
        if option in text:
            return option
    return "独立开发"
...
def build_fields(
    ...
    defaults = {
        ...
        "开发方式": normalize_development_situation(business.get("development_situation") if business else None),
        "软件说明": "原创",
        "发表状态": "待用户确认（已发表/未发表）",
    ...
    # 归一化开发方式：兼容旧口径"单独开发"，统一为官方选项"独立开发"
    dev_value = defaults.get("开发方式", "")
    if dev_value and "待用户确认" not in dev_value:
        defaults["开发方式"] = normalize_development_situation(dev_value)
...
def write_application_md(path: Path, fields: dict[str, str], analysis: dict[str, Any], manifest: dict[str, Any], business: dict[str, Any] | None = None) -> None:
    ...
    lines.extend(
        [
            ...
            "- 开发方式：独立开发/合作开发/委托开发/下达任务开发（官方无“单独开发”，默认独立开发）。",

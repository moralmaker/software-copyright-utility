...
def parse_screenshot_method(method: str, note: str) -> str:
    ...
    if any(key in value for key in ("skip", "no-screenshot", "none", "不截图", "跳过", "暂不", "先不", "不要截图", "无需截图")):
        return "skip"
    if any(key in value for key in ("wechat", "weapp", "weixin", "小程序", "微信")):
    ...
    raise SystemExit(
        "STOP_FOR_USER\n"
        "NEXT_ACTION: 请明确截图方式：chrome-devtools、wechat-devtools（小程序）、computer-use、user-supplied 或 skip。"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--method",
        choices=["chrome-devtools", "wechat-devtools", "computer-use", "user-supplied", "skip"],

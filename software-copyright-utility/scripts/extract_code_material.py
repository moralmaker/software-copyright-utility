...
# 单行显示宽度上限（按显示单位计：ASCII/半角算 1、CJK/全角算 2）。
# 超过则硬折行，保证每行不超出 A4 可打印宽度，避免长行折行导致整页溢出、页数虚增。
MAX_LINE_WIDTH = 72
...
def wrap_code_line(line: str, width: int = MAX_LINE_WIDTH) -> list[str]:
    """把一行源码按显示宽度硬折成多段，每段 ≤ width 显示单位（CJK 记 2）。"""
    if not line:
        return [""]
    parts: list[str] = []
    cur = ""
...
def wrap_code_lines(lines: list[str], width: int = MAX_LINE_WIDTH) -> list[str]:
    out: list[str] = []
    for line in lines:
...
# C 风格（// 与 /* */）注释语言；软著源代码要求去除注释，参考《源代码-模板》。
_C_STYLE_EXTS = {
    ".java", ".js", ".ts", ".tsx", ".jsx", ".c", ".h", ".cpp", ".cc", ".cxx",
    ".hpp", ".hh", ".cs", ".go", ".kt", ".kts", ".swift", ".scala", ".groovy",
}


def _strip_c_comments(text: str) -> str:
    """去除 C 风格（Java/JS/TS/C/C++/Kotlin/Swift/Go/C# 等）注释。

    保留字符串与字符字面量内部出现的 // 与 /* 原样（如 "http://x"），
    删除行内注释与块注释，并保留原换行结构（块注释跨行处保留换行，
    之后整行变空会被上层过滤）。
    """
    out: list[str] = []
    i, n = 0, len(text)
...
def _strip_vue_comments(text: str) -> str:
    # .vue 单文件组件：先删 HTML 注释 <!-- ... -->（跨行），
    # 再删 <script> 的 // 与 /* */、<style> 的 /* */ 注释（保留字符串内 //）。
    text = re.sub(r"<!--.*?-->", "", text, flags=re.S)
    return _strip_c_comments(text)


def material_code_lines(text: str, path: Path) -> list[str]:
    # 软著源代码不含注释：按模板剥离各语言注释，再丢弃空行。
    ext = path.suffix.lower()
    if ext == ".vue":
        text = _strip_vue_comments(text)
    elif ext in _C_STYLE_EXTS:
...
def collect_code_lines(project: Path, selection_path: Path | None) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    for item in selected_items:
        ...
        selected_lines = material_code_lines(text, path)
...
def extract(project: Path, out_dir: Path, software_name: str, version: str, lines_per_page: int, selection_path: Path | None) -> dict[str, Any]:
    ...
    # 先按显示宽度折行长行，再按“显示行”分页，保证每个“第 N 页”恰好 50 显示行、
    # 且不超出可打印宽度，从而每页渲染为一张物理页（前30页=30页、后30页=30页）。
    display_lines = wrap_code_lines(code_lines)
    pages = paginate(display_lines, lines_per_page)
